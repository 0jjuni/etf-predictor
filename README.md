...
지난 N거래일을 walk-forward로 채워넣고 싶을 때:

```powershell
# .env에 SUPABASE_URL, SUPABASE_SERVICE_KEY 들어있어야 함
$env:XGB_DEVICE = "cuda"            # GPU 있으면 (없으면 생략 → cpu)
uv run python scripts/backfill.py --days 7
```

각 날짜 D에 대해:
- D 이전 데이터로만 모델 재학습 (target-date leakage 없음)
- D를 위한 예측 생성 + D의 실제 종가로 outcome 자동 fill
- predictions / model_metrics 모두 Supabase에 적재

> 7번 재학습이라 CPU에선 오래 걸려요. CUDA 권장. 클라우드(GH Actions)에서 절대 돌리지 말 것 — 무료 분 다 씀.
