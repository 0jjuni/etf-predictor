    ...
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/85 backdrop-blur">
      <nav className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3">
        ...
        >
          ...
          <span className="group-hover:text-indigo-600">ETF 종가 예측기</span>
        </Link>
...
function NavLink({ href, label }: { href: string; label: string }) {
  return (
    <li>
      <Link
        className="rounded-md px-2.5 py-1.5 transition hover:bg-slate-100 hover:text-indigo-600 sm:px-3"
        href={href}
      >
        {label}
      </Link>
    ...
    <footer className="border-t border-slate-200 bg-white">
      <div className="mx-auto max-w-5xl px-4 py-8 text-xs text-slate-500">
        <p className="leading-relaxed">
          본 서비스는{" "}
          <strong className="text-slate-700">투자 판단의 보조 자료</strong>일
          뿐이며 매수·매도 권유가 아닙니다. 투자 결과의 책임은 전적으로 투자자
          본인에게 있습니다.
        </p>
        <p className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5">
          <Link className="hover:text-indigo-600 hover:underline" href="/about">
            소개
          </Link>
          <Link
