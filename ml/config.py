...
# Supabase so the UI can show a precision band for each predicted ETF. Starts
# below PROB_THRESHOLD so fallback picks (when nothing crosses 0.70) still get
# meaningful precision context.
THRESHOLD_GRID: list[float] = [round(0.30 + 0.05 * i, 2) for i in range(14)]
